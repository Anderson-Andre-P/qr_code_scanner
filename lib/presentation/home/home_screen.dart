...
import 'package:qr_code_scanner/models/bank_info.dart';
import 'package:qr_code_scanner/models/wifi_info.dart';
...
class _QRHomePageState extends State<QRHomePage>
  ...
  void copyToClipboard(String text, ctx) {
    ...
    CustomSnackbar.show(ctx, 'Texto copiado para a área de transferência');
  }
  ...
  Future<void> saveToGallery(ctx) async {
    try {
      ...
      if (result['isSuccess']) {
        CustomSnackbar.show(ctx, 'Imagem salva na galeria');
      } else {
        CustomSnackbar.show(ctx, 'Falha ao salvar a imagem');
      }
    } catch (e) {
      CustomSnackbar.show(ctx, 'Falha ao salvar a imagem');
    }
  ...
  void shareCode(ctx) async {
    try {
      ...
      CustomSnackbar.show(ctx, 'Falha ao compartilhar o código QR');
    }
  }
                  ...
                  children: [
                    ...
                    MaterialButton(
                      ...
                      onPressed: () {
                        ...
                        CustomSnackbar.show(context, 'QRCode Gerado!',
                            color: Colors.green);
                      },
                    ),
                  ],
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  'Toque no botão abaixo para gerar códigos QR',
    ...
    if (qrCodeData != '' && qrCodeData.isNotEmpty) {
      try {
        ...
        if (match != null) {
          ...
          CustomSnackbar.show(context, 'Wi-Fi QR code Inválido');
        }
      } catch (e) {
        CustomSnackbar.show(context, 'Erro ao extrair a senha do Wi-Fi: QR');
      }
    }
    ...
    if (qrCodeData != '' && qrCodeData.isNotEmpty) {
      try {
        ...
        if (match != null) {
          ...
          CustomSnackbar.show(context, 'QR Inválido');
        }
      } catch (e) {
        CustomSnackbar.show(
            context, 'Erro ao extrair informações bancárias: QR');
      }
    }
  ...
  void _foundCode(BarcodeCapture barcode) {
    ...
    if (!_screenOpened) {
      final code = barcode.barcodes[0].rawValue ?? "Nenhum código disponível";
      ...
      CustomSnackbar.show(context, 'QRCode Gerado!', color: Colors.green);
  ...
  void _startOrStop(ctx) {
    try {
      ...
      CustomSnackbar.show(ctx, 'Algo deu errado! $e', color: Colors.red);
    }
  }
                ...
                onPressed: () async {
                  ...
                  if (image != null) {
                    if (await cameraController.analyzeImage(image.path)) {
                      ...
                      CustomSnackbar.show(context, 'Nenhum QRCode encontrado!',
                          color: Colors.red);
                    }
                  }
                },
